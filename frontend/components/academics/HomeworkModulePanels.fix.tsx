...
function constructFileUrl(filePath: string | undefined): string {
  ...
  // If starts with /, it's a relative root path
  if (filePath.startsWith("/")) {
    return filePath;
  }
  // Otherwise prepend /media/ for uploaded files
        ...
        <div className="container-fluid p-0">
          ...
          {viewingHomework && (
            <div className="white-box" style={{ ...boxStyle(), marginTop: 12 }}>
              <h3 style={{ marginTop: 0 }}>Homework Details</h3>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Homework Date</label>
                  <div style={{ fontSize: 14 }}>{viewingHomework.homework_date}</div>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Submission Date</label>
                  <div style={{ fontSize: 14 }}>{viewingHomework.submission_date}</div>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Marks</label>
                  <div style={{ fontSize: 14 }}>{viewingHomework.marks}</div>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Evaluation Date</label>
                  <div style={{ fontSize: 14 }}>{viewingHomework.evaluation_date || "-"}</div>
                </div>
              </div>
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Description</label>
                <div style={{ fontSize: 14, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{viewingHomework.description}</div>
              </div>
              {viewingHomework.file && (
                <div style={{ marginBottom: 12 }}>
                  <label style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>Attachment</label>
                  <a href={constructFileUrl(viewingHomework.file)} target="_blank" rel="noopener noreferrer" style={{ color: "var(--primary)", textDecoration: "underline" }}>{viewingHomework.file}</a>
                </div>
              )}
              <div style={{ marginTop: 12 }}>
                <button type="button" onClick={() => setViewingHomework(null)} style={buttonStyle("#6b7280")}>Close</button>
              </div>
            </div>
          )}
