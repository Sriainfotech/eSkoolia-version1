...
import { getImageNaturalSize } from "@/lib/document-branding";
...
export function HrPayrollPanel() {
  ...
  // Signature stays payslip-specific (no other document type needs one) —
  // everything else about the header comes from Settings > Document
  // Branding, the one central source every print/PDF feature uses.
  const [payslipSignatureUrl, setPayslipSignatureUrl] = useState("");
  const { headerImageDataUrl: payslipHeaderImage, declarationText: payslipDeclaration } = useDocumentBranding(
    "payslip",
  ...
  const downloadPayrollPdf = async (row: PayrollRecord) => {
    ...
    // School header — Settings > Document Branding, the one central source
    // every print/PDF feature in the app uses.
    if (payslipHeaderImage) {
      try {
        const { width: natW, height: natH } = await getImageNaturalSize(payslipHeaderImage);
        const headerH = Math.min(22, 182 * (natH / natW));
        doc.addImage(payslipHeaderImage, "PNG", 14, y, 182, headerH);
        y += headerH + 6;
      } catch {
        doc.setFontSize(16);
        doc.text(payslipSchoolName || "Payroll Payslip", 14, y + 6);
        y += 13;
      }
    } else {
      doc.setFontSize(16);
    ...
    const declarationLines = doc.splitTextToSize(payslipDeclaration, 182);
    doc.text(declarationLines, 14, y + 102);
    doc.text("Generated by Eskoolia Payroll", 14, y + 102 + declarationLines.length * 4 + 6);
    doc.setTextColor(17, 24, 39);
...
<head>
  ...
  <style>
    ...
    .declaration { margin-top: 18px; font-size: 11.5px; color: #6b7280; line-height: 1.6; padding: 10px 12px; background: #f9fafb; border-radius: 6px; }
    .header-img { display: block; max-width: 100%; max-height: 90px; object-fit: contain; object-position: left center; margin-bottom: 14px; }
...
<body>
  <div class="card">
    ${payslipHeaderImage
      ? `<img src="${payslipHeaderImage}" alt="School header" class="header-img" />`
      : `<p class="title">${payslipSchoolName || "Payroll Payslip"}</p>`}
    <div class="header">
      <p class="sub" style="margin:0;">Month ${row.payroll_month} / ${row.payroll_year}</p>
    ...
    <p class="declaration">${payslipDeclaration}</p>
