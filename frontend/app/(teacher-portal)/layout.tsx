 ...
 *
 * Matches the admin ERP layout exactly:
 *   - Same TopBar visual style (logo, module pills, search, avatar)
 *   - Same ModuleSubNav sub-header below TopBar
 *   - No sidebar — module navigation is in the top bar
 *
 * Module pills use TEACHER_MODULES so ModulePill is reused as-is.
 * Sub-nav tabs are filtered by usePermissions() — same as admin console.
 */
...
function TeacherTopBar() {
  ...
  const initials = fullName
    .split(" ").filter(Boolean).map((w) => w[0]).slice(0, 2).join("").toUpperCase() || "T";

  const isHome = pathname === "/teacher/home";
  ...
  return (
    <header style={{
      ...
      <div style={{
        ...
        {/* Logo */}
        <Link href="/teacher/home" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", flexShrink: 0 }}>
          <span style={{
            width: 32, height: 32, borderRadius: 9,
            background: "linear-gradient(135deg, var(--pu) 0%, var(--pu-deep) 100%)",
            display: "flex", alignItems: "center", justifyContent: "center",
            color: "#fff", fontWeight: 700, fontSize: 18, letterSpacing: "-0.04em", flexShrink: 0,
          }}>
            e
          </span>
          <span style={{ fontWeight: 600, fontSize: 17, letterSpacing: "-0.02em", color: "var(--ink-1)" }}>
            eskoolia
          </span>
        </Link>
        ...
        <div style={{
          ...
          textTransform: "uppercase", color: "var(--pu)",
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 2, marginLeft: 4, flex: "1 1 0", minWidth: 0, overflow: "hidden" }}>
          ...
          <div
            ref={navScrollRef}
            style={{ display: "flex", alignItems: "center", gap: 1, flex: 1, overflowX: "auto", scrollbarWidth: "none" }}
          >
            {navModules.map((mod) => (
              <ModulePill key={mod.id} mod={mod} exactMatch />
            ))}
          </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          <button style={{
            ...
            background: "var(--bg-2)", color: "var(--ink-3)", fontSize: 12,
          ...
          >
            ...
            <kbd style={{
              ...
              border: "1px solid var(--bd)", borderRadius: 4, padding: "1px 5px", lineHeight: 1.4,
            }}>⌘K</kbd>
          </button>
          ...
          <div ref={avatarRef} style={{ position: "relative", flexShrink: 0 }}>
            ...
            {avatarOpen && (
              <div style={{
                ...
                width: 190, background: "var(--bg-1)",
                border: "1px solid var(--bd)", borderRadius: 12,
                boxShadow: "var(--sh-3)", padding: 6, zIndex: 100,
              }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12.5, color: "var(--err)", cursor: "pointer",
                  }}
                >
                  Logout
                </button>
...
// ── Teacher SubNav ────────────────────────────────────────────────────────────
// Matches ModuleSubNav exactly — shows sub-tabs for the current teacher module.

function TeacherSubNav() {
  const pathname = usePathname();
  const scrollRef = useRef<HTMLDivElement>(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(false);
  const { can } = usePermissions();
  // All modules this teacher can see — same list used for nav pills
  ...
  const mod = allMods.find(
    (m) =>
      ...
      (m.path !== "/teacher/home" && pathname.startsWith(m.path))
  );
  ...
  const arrowBtn = (dir: "left" | "right", enabled: boolean) => (
    <button
      ...
      disabled={!enabled}
      style={{
        flexShrink: 0, width: 28, height: 46, border: "none",
        cursor: enabled ? "pointer" : "default",
        background: enabled
          ? `linear-gradient(to ${dir === "left" ? "right" : "left"}, #fff 60%, transparent)`
          : "transparent",
        display: "flex", alignItems: "center", justifyContent: "center",
        opacity: enabled ? 1 : 0.25, padding: 0,
      }}
    >
      {dir === "left"
    ...
    <div style={{ background: "#fff", borderBottom: "1px solid var(--bd, #dbe4f0)" }}>
      <div style={{
        ...
        <div style={{
          ...
          borderRight: "1px solid var(--bd, #dbe4f0)", flexShrink: 0,
        }}>
          <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-2, #475569)", whiteSpace: "nowrap" }}>
            {mod.name}
          </span>
        </div>
        ...
        <div ref={scrollRef} style={{
          ...
          scrollbarWidth: "none", gap: 0, flex: 1,
        }}>
          {mod.sub.map((s) => {
            const isActive = s === activeTab;
            const SubIcon = s.icon ?? mod.icon;
            const allowed = s.permission ? can(s.permission) : true;
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  fontSize: 12.5, fontWeight: isActive ? 600 : 400, lineHeight: 1.25,
                ...
                onMouseEnter={(e) => {
                  if (!isActive) (e.currentTarget as HTMLAnchorElement).style.color = "var(--ink-1, #0f172a)";
                }}
                onMouseLeave={(e) => {
                  if (!isActive) (e.currentTarget as HTMLAnchorElement).style.color = "var(--ink-2, #475569)";
                }}
              >
