 ...
 *
 * Same card pattern as app/(parent-portal)/parent/notices/page.tsx, calling
 * GET /api/v1/teacher/notices/ instead — kept as a near-identical twin
 * rather than a shared component so each portal's copy can drift
 * independently (e.g. teacher notices may later grow a "mark as read" or
 * priority-sort feature parents don't need).
 */

import { useEffect, useState } from "react";
import { Bell, ChevronDown, ChevronUp } from "lucide-react";
import { fetchTeacherNotices, type NoticeItem } from "@/lib/api/teacher";
...
function NoticeCard({ notice }: { notice: NoticeItem }) {
  ...
  return (
    <div style={{ border: "1px solid var(--bd)", borderRadius: 14, overflow: "hidden", transition: "box-shadow 0.15s" }}>
      ...
      >
        <div style={{ width: 38, height: 38, borderRadius: 10, background: "#FFFBEB", display: "grid", placeItems: "center", flexShrink: 0, marginTop: 1 }}>
          <Bell size={16} color="#B45309" strokeWidth={1.8} />
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
            ...
            {isNew && (
              <span style={{ padding: "2px 8px", borderRadius: 20, background: "#FEF3C7", color: "#B45309", fontSize: 10, fontWeight: 700 }}>NEW</span>
            )}
          </div>
    ...
    <div style={{ background: "var(--bg-1)", border: "1px solid var(--bd)", borderRadius: 18, boxShadow: "var(--sh-1)", padding: "28px 30px" }}>
      ...
      <div style={{ marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid var(--bd)", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 34, fontWeight: 600, color: "var(--ink-1)", margin: "0 0 5px", lineHeight: 1.05, letterSpacing: "-0.03em", display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
            School{" "}
            <em style={{ fontFamily: "var(--font-instrument-serif,'Instrument Serif',Georgia,serif)", fontWeight: 400, fontStyle: "italic", color: "#B45309", fontSize: 38, letterSpacing: "-0.02em" }}>
              Notices
            </em>
          </h1>
          <p style={{ margin: 0, fontSize: 13, color: "var(--ink-2)", marginTop: 10, lineHeight: 1.55 }}>
            Announcements and circulars from your school.
          </p>
        </div>
        {notices.length > 0 && (
      ...
      {loading ? (
        ...
        <div style={{ background: "var(--bg-2)", border: "1px solid var(--bd)", borderRadius: 12, padding: "40px 32px", textAlign: "center" }}>
          <div style={{ width: 48, height: 48, borderRadius: 14, background: "#FFFBEB", display: "grid", placeItems: "center", margin: "0 auto 12px" }}>
            <Bell size={22} color="#B45309" strokeWidth={1.5} />
          </div>
