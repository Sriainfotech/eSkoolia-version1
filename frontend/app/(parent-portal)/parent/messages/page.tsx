 ...
 *
 * Same inbox shell as the teacher-side Messages page, calling the parent
 * endpoints instead. Live push reuses the same portal-agnostic
 * usePortalNotifications hook (it just needs a valid token — works for
...
import { fetchParentMessages, type InAppMessageItem } from "@/lib/api/parent";
import ComposeParentMessageModal from "@/components/parent/ComposeParentMessageModal";
...
export default function ParentMessagesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError(false);
    fetchParentMessages().then(setMessages).catch(() => setError(true)).finally(() => setLoading(false));
  }, []);
    ...
    <div style={{ background: "var(--bg-1)", border: "1px solid var(--bd)", borderRadius: 18, boxShadow: "var(--sh-1)", padding: "28px 30px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 22, paddingBottom: 20, borderBottom: "1px solid var(--bd)" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "var(--ink-2)", marginTop: 10, lineHeight: 1.55 }}>Messages with your child&rsquo;s teachers — updates arrive live.</p>
        </div>
        <button onClick={() => openCompose(undefined)} disabled={!selectedChild}
          style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 18px", borderRadius: 9, border: "none", background: "#0369A1", color: "#fff", fontSize: 13, fontWeight: 600, cursor: selectedChild ? "pointer" : "default", opacity: selectedChild ? 1 : 0.6 }}>
      ...
      {loading ? (
        ...
        <div style={{ background: "var(--bg-2)", border: "1px solid var(--bd)", borderRadius: 12, padding: 40, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12.5, color: "var(--ink-3)" }}>Start a conversation with a teacher using &ldquo;New Message&rdquo;.</div>
        </div>
      ) : (
