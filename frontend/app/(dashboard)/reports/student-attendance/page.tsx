...
export default function StudentAttendanceReportPage() {
  ...
  const search = async () => {
    ...
    setError("");
    try {
      const data = await apiFetch<{ count: number; results: AttendanceRow[] }>(`/api/v1/reports/students/attendance/?${buildParams()}`);
  ...
  const exportCsv = () => {
    const token = getAccessToken();
    window.open(`${API_BASE_URL}/api/v1/reports/students/attendance/?${buildParams()}&format=csv&token=${token ?? ""}`, "_blank");
  };
