...
function generateAbsentReason(staffName: string): string {
  const templates = [
    `${staffName} is on approved personal leave today.`,
    `${staffName} is unwell and has informed HR in advance.`,
    `${staffName} has a medical appointment and is expected to rejoin tomorrow.`,
    `${staffName} reported a transport issue and could not attend today.`,
  ];
...
export default function StaffAbsentNoteDialog({ staff, initialReason, onConfirm, onSkip }: Props) {
        ...
        <div className="px-5 py-4 border-b border-[#F0F0F6]">
          ...
          <div className="ml-7 mt-2">
            <button
              ...
              onClick={() => {
                setSelected(null);
                setCustom(generateAbsentReason(staffName));
              }}
