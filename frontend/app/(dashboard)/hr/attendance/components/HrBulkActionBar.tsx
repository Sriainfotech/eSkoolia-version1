...
interface Props {
  count: number;
  onClear: () => void;
  onMarkAll: (status: "P" | "A") => void;
  onSignInAll: () => void;
  onSignOutAll?: () => void;
}

export default function HrBulkActionBar({ count, onClear, onMarkAll, onSignInAll, onSignOutAll }: Props) {
    ...
    <div className="flex items-center gap-2.5 flex-wrap px-5 py-2 bg-[#0B0B14] min-h-[40px]">
      ...
      <div className="flex gap-1.5 ml-2.5">
        <button
          ...
          title="Sign in selected staff and mark them present"
        >
